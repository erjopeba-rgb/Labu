...
- ✅ Validación de inputs con express-validator — **completada en I4 del AUDIT.md (2026-06-12)**: cadenas declarativas en todas las routes de escritura (pagos, disputas, disponibilidad, admin, ayudantes, calificaciones, confianza, jobs, offers) + helper central `validacion.middleware.js` (`manejarErrores`, `validarIdParam` — fin de los `parseInt(param)` → NaN → 500)
...
- ✅ Tests: 187/187 pasando (Jest 30) — sin dependencia de red externa (OSRM y MercadoPago mockeados); cobertura nueva I13: firma HMAC del webhook MP, CRUD de disputas, disponibilidad/reservas
...
- **2026-06-12 tarde:** **I4, I12, I13 resueltos** (último paquete de importantes del AUDIT.md) — **I4**: helper central `backend/middlewares/validacion.middleware.js` (`manejarErrores` + `validarIdParam` + `REGEX_HORA`) y cadenas express-validator en todas las routes de escritura: pagos (montos, card_token/payer_email, retiros; webhook excluido a propósito — contrato 200/500 con MP), disputas (trabajo_id/motivo/evidencia), disponibilidad (dia_semana 0–6, horas HH:MM, reservas), admin (whitelists de accion/resultado, nota obligatoria al rechazar retiro vía `.if()`), ayudantes, calificaciones, confianza; jobs/offers alineados al helper y todos los params `:id` validados (fin de `parseInt` → NaN → 500). **I12**: aserciones exactas en `auth.test.js` (reset token inválido → 400), `offers.test.js` (oferta propia → 400, aceptar aceptada → 404), `reportes.test.js` separado en 201 vs 409 autocontenidos; OSRM mockeado en `scheduling.test.js` (`obtenerTiempoViaje` exportada con indirección `module.exports` + spy 20 min = 1200 s, test nuevo verifica slot 09:00 → inicio 09:35 = prep 15 + viaje 20). **I13 (parcial)**: firma HMAC del webhook (5 tests: válida procesa, inválida/ausente/malformada/data.id cruzado → 401), `disputas.test.js` nuevo (13 tests: 201/400/403/409, permisos de lectura, evidencia; no existe endpoint de mensajes de disputa — solo la tabla), `disponibilidad.test.js` nuevo (17 tests: config semanal, reservas tentativas, confirmación de slot con liberación; "conflicto → 409" no existe por diseño: re-crear reservas reemplaza tentativas y el conflicto real lo previene C1). Tests: 187/187 (14 suites, sin red externa).
